...
public class StringsBuildUtil {
	final static String[] reservedWordList = { "abstract", "boolean", "break", "byte", "case",
	...
	static {
		...
		// System.out.println("all " + reservedWordSet.size() +
		// " reserved words loaded");
	}
	...
	public static String escapeSystemKeyword(String word, boolean inSql) {
		...
		if (fieldName.startsWith("_")) {
			...
			if (isSystemKeyword(possibleWord)) {
				if (inSql)
					fieldName = new StringBuilder().append('`').append(possibleWord)
							.append('`').toString();
				else
	...
	public static String getFullJavaName(Class<?> c) {
		return c.getName();
	}

	public static String getShortJavaName(Class<?> c) {
